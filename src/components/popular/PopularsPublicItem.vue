...
<div class="main-page" v-if="getPostFilm">
    <div class="main-content">
      <div class="mp-inner">
        ...
        <div class="post">
          <div class="post-img">
            <img v-if="getPostFilm.img" :src="require('@/assets/images/popular/' + getPostFilm.img)" class="post shadow" />
            <img v-else :src="require('@/assets/no-poster.jpeg')" alt="" >
          </div>
          <div class="post-item">
            <div class="post-img" >
              <img v-if="getPostFilm.img" :src="require('@/assets/images/popular/' + getPostFilm.img)" style="height: 100%" />
              <img v-else :src="require('@/assets/no-poster.jpeg')" alt="" style="height: 100%">
            </div>
          </div>
        </div>
      </div>
            ...
            <div v-for="actor in getPostFilm.actors" :key="actor.id" class="mp-actor-item" @click="seeTheActor(actor.article)">
              <div class="mp-post">
                <div class="mp-post-img">
                  <img :src="require('@/assets/images/popular/actors/' + actor.img)" alt="" v-if="actor.img" class="mp-post shadow" />
                  <img :src="require('@/assets/no-poster.jpeg')" alt="" v-else />
                </div>
                <div class="mp-post-item">
                  <div class="mp-post-img">
                    <img :src="require('@/assets/images/popular/actors/' + actor.img)" alt="" v-if="actor.img"  />
                    <img :src="require('@/assets/no-poster.jpeg')" alt="" v-else />
                  </div>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    seeTheActor(article) {
      this.$router.push({name: 'popularsActor', query:{ 'actor': article }})
    },
    transitionTrailer() {
      this.$router.push({name: 'popularsTrailer', query: { 'card': this.getPostFilm.article}})
    },
    transitionFilm() {
      this.$router.push({name: 'popularsVideo', query: { 'card': this.getPostFilm.article}})
    }
  },
 async mounted() {
     await this.getFilms()
