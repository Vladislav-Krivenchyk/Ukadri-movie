  ...
  <div class="main-page" v-if="getFilm">
    <div class="main-content">
      <div class="mp-inner">
        <div class="mp-episode">
          <div class="mp-description">
            ...
            <div class="post">
              <div class="post-img">
                <img :src="require('@/assets/images/serials/postItem/' + getFilm.imgTop)" class="post shadow" />
              </div>
              <div class="post-item">
                <div class="post-img" >
                  <img v-if="getFilm.imgTop" :src="require('@/assets/images/serials/postItem/' + getFilm.imgTop)"  />
                </div>
              </div>
      ...
      <div class="background-section" v-if="getFilm.imgTop">
        <img :src="require('@/assets/images/serials/postItem/' + getFilm.imgTop)" style="width: 100%; height: 1200px" />
      </div>
      <div class="background-section" v-if="img">
        <img :src="require('@/assets/images/serials/postItem/' + img)" style="width: 100%; height: 1200px" />
      </div>
        ...
        <slide v-for="(slide, index ) in getCatsCard" :key="slide.id" :index="index" >
          <template slot-scope="{ index, isCurrent, leftIndex, rightIndex }">
            <img  :data-index="index"
                  :src="require('@/assets/images/serials/' + slide?.img )"
...
export default {
  ...
  computed: {
    ...mapGetters('serialsStore', ['getSerialsState']),

    getCatsCard() {
      return this.getSerialsState
    },
    getFilm() {
      let res = {}
      this.getSerialsState.find(item => {
  ...
  methods: {
    ...
    goToTheMovie() {
      this.$router.push({name: 'serialsVideo', query: { 'card': this.getFilm.article}})
    },
    goToTheFilmWithSliders() {
      this.$router.push({name: 'serialsPublicItem', query: { 'card': this.article}})
    }
  },
  async mounted() {
