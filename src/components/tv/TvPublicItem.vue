  ...
  <div class="main-page">
    <div class="main-content">
      <div class="mp-inner">
        ...
        <div class="mp-img" >
          <img :src="require('@/assets/images/tv/' + getPostFilm.img)" alt="" v-if="getPostFilm.img">
      ...
      <div class="mp-actors">
        <div v-for="actor in getPostFilm.actors" :key="actor.id" class="mp-actor-item" @click="seeTheActor(actor.article)">
          <img :src="require('@/assets/images/tv/actors/' + actor.img)" alt="" v-if="actor.img" />
          <img :src="require('@/assets/no-poster.jpeg')" alt="" v-else />
          <h4>{{actor.name}}</h4>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    seeTheActor(article) {
      this.$router.push({name: 'tvActor', query:{ 'actor': article }})
    },
    transitionTrailer() {
      this.$router.push({name: 'tvTrailer', query: { 'card': this.getPostFilm.article}})
    },
    transitionFilm() {
      this.$router.push({name: 'tvVideo', query: { 'card': this.getPostFilm.article}})
    }
  },
  async mounted() {
    await this.getTv()
