  ...
  <div class="main-page" v-if="getPostFilm">
    <div class="main-content">
      <div class="mp-inner">
        ...
        <div class="post">
          <div class="post-img">
            <img v-if="getPostFilm.img" :src="require('@/assets/images/shou/' + getPostFilm.img)" class="post shadow" />
            <img v-else :src="require('@/assets/no-poster.jpeg')" alt="" >
          </div>
          <div class="post-item">
            <div class="post-img" >
              <img v-if="getPostFilm.img" :src="require('@/assets/images/shou/' + getPostFilm.img)"  />
              <img v-else :src="require('@/assets/no-poster.jpeg')" alt="">
            </div>
          </div>
        </div>
        ...
        <div v-for="actor in getPostFilm.actors" :key="actor.id" class="mp-actor-item" @click="seeTheActor(actor.article)">
          <div class="mp-post">
            <div class="mp-post-img">
              <img :src="require('@/assets/images/shou/actors/' + actor.img)" alt="" v-if="actor.img" class="mp-post shadow" />
              <img :src="require('@/assets/no-poster.jpeg')" alt="" v-else />
            </div>
            <div class="mp-post-item">
              <div class="mp-post-img">
                <img :src="require('@/assets/images/shou/actors/' + actor.img)" alt="" v-if="actor.img"  />
                <img :src="require('@/assets/no-poster.jpeg')" alt="" v-else />
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    seeTheActor(article) {
      this.$router.push({name: 'shouActor', query:{ 'actor': article }})
    },
    transitionTrailer() {
      this.$router.push({name: 'shouTrailer', query: { 'card': this.getPostFilm.article}})
    },
    transitionFilm() {
      this.$router.push({name: 'shouVideo', query: { 'card': this.getPostFilm.article}})
    }
  },
  async mounted() {
    await this.getSport()
