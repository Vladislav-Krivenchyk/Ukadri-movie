  ...
  <div class="main-page" v-if="getFilm">
    <div class="main-content">
      <div class="mp-inner">
        <div class="mp-episode">
          <div class="mp-description">
            ...
            <div class="post">
              <div class="post-img">
                <img :src="require('@/assets/images/shou/postItem/' + getFilm.imgTop)" class="post shadow" />
              </div>
              <div class="post-item">
                <div class="post-img" >
                  <img v-if="getFilm.imgTop" :src="require('@/assets/images/shou/postItem/' + getFilm.imgTop)"  />
                </div>
              </div>
      ...
      <div class="background-section" v-if="getFilm.imgTop">
        <img :src="require('@/assets/images/shou/postItem/' + getFilm.imgTop)" style="width: 100%; height: 1200px" />
      </div>
      <div class="background-section" v-if="img">
        <img :src="require('@/assets/images/shou/postItem/' + img)" style="width: 100%; height: 1200px" />
      </div>
        ...
        <slide v-for="(slide, index ) in getCatsCard" :key="slide.id" :index="index" >
          <template slot-scope="{ index, isCurrent, leftIndex, rightIndex }">
            <img  :data-index="index"
                  :src="require('@/assets/images/shou/' + slide?.img )"
...
export default {
  ...
  computed: {
    ...mapGetters('shouStore', ['getShouState']),

    getCatsCard() {
      return this.getShouState
    },
    getFilm() {
      let res = {}
      this.getShouState.find(item => {
  ...
  methods: {
    ...
    goToTheTrailer() {
      this.$router.push({name: 'shouTrailer', query: { 'card': this.getFilm.article}})
    },
    goToTheMovie() {
      this.$router.push({name: 'shouVideo', query: { 'card': this.getFilm.article}})
    },
    goToTheFilmWithSliders() {
      this.$router.push({name: 'shouPublicItem', query: { 'card': this.article}})
    }
  },
  async mounted() {
    await this.getShou()
